...
class ProductAdapter(
    ...
    private var onProductBought: (Product) -> Unit // New callback for bought notification
    ...
    override fun onBindViewHolder(holder: ProductViewHolder, position: Int) {
        ...
        holder.boughtButton.backgroundTintList = ContextCompat.getColorStateList(
            context,
            if (product.isBought) R.color.button_selected else R.color.button_pressed
        )
        holder.boughtButton.setOnClickListener {
            onBoughtClick(product)
            if (!product.isBought) { // Notify only when marking as bought
        ...
        holder.itemView.contentDescription = "Product: ${product.name}, added by ${product.addedBy} on ${dateFormat.format(Date(product.timestamp))}: ${if (product.isBought) "bought" else "not bought"}, ${productName} downvotes"
    }
    ...
    fun setCallbacks(
        onBoughtClick: (Product) -> Unit,
        onDownvoteClick: (Product) -> Unit,
        onProductBought: (Product) -> Unit
    ) {
        ProductAdapter.onBoughtClick = onBoughtClick
